...
func (ts *AuditTestSuite) makeSuperAdmin(email string) string {
	u, err := models.NewUser("TEST0101", "", email, "test", ts.Config.JWT.Aud, map[string]interface{}{"full_name": "Test User"})
...
func (ts *AuditTestSuite) prepareDeleteEvent() {
	// DELETE USER
	u, err := models.NewUser("TEST0101", "12345678", "test-delete@example.com", "test", ts.Config.JWT.Aud, nil)
	require.NoError(ts.T(), err, "Error making new user")
