...
WORKDIR /go/src/github.com/cybinon/auth
...
# Building stuff
COPY . /go/src/github.com/cybinon/auth
...
COPY --from=build /go/src/github.com/cybinon/auth/auth /usr/local/bin/auth
COPY --from=build /go/src/github.com/cybinon/auth/migrations /usr/local/etc/auth/migrations/
